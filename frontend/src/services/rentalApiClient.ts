...
const rentalApiClient = axios.create({
  baseURL: import.meta.env.VITE_RENTAL_API,
});

rentalApiClient.interceptors.request.use((config) => {
...
rentalApiClient.interceptors.response.use(
  (response) => response,
  (error) => {
...
export default rentalApiClient;
