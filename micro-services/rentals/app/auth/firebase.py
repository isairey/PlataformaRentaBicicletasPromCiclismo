...
def initialize_firebase(app) -> None:
    ...
    missing_fields = [f for f in required_fields if not certificate_data[f]]

    if missing_fields:
        if app.config.get("TESTING"):
            return
        missing_env_vars = ", ".join(f"FIREBASE_{f.upper()}" for f in missing_fields)
        raise RuntimeError(f"Firebase environment variables are missing: {missing_env_vars}.")
